...
func getRequest(conn *ss.Conn, auth bool) (host string, ota bool, err error) {
	...
	/*
		数据模型
		0x1	IP类型 1=IPV4 4=IPV6 3=域名
	*/
	//log.Println("getRequest data ", buf[idType])
	...
	default:
		log.Println("getRequest addr type %d not supported", addrType&ss.AddrMask)
	...
	if auth || addrType&ss.OneTimeAuthMask > 0 {
		...
		if !bytes.Equal(buf[reqEnd:reqEnd+lenHmacSha1], actualHmacSha1Buf) {
			log.Println("getRequest verify one time auth failed, iv=%v key=%v data=%v", iv, key, buf[:reqEnd])
...
func runUDP(port, password string, auth bool) {
	var cipher *ss.Cipher           // 加密变量
	port_i, _ := strconv.Atoi(port) // 转换端口
	log.Printf("listening udp port %v\n", port)
	conn, err := net.ListenUDP("udp", &net.UDPAddr{ // 绑定监听udp端口
		//IP:   net.IPv6zero, 备注
		IP:   net.IPv4zero,
		Port: port_i,
	})
	passwdManager.addUDP(port, password, conn) // 添加密码管理
	if err != nil {
