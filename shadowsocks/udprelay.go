...
func handleUDPConnection(handle *SecurePacketConn, n int, src net.Addr, receive []byte) {
	...
	//log.Println("handleUDPConnection", receive, receive[idType])
	/*
		数据结构
		1.IP类型 1=IPV4 4=IPV6 3=域名
		2.IP IP地址 4位
		3.Port 端口 2位
		4.data ...
	*/
	...
	tmpPort := int(binary.BigEndian.Uint16(receive[reqLen-2 : reqLen]))
	dst := &net.UDPAddr{
		IP:   dstIP,
		Port: tmpPort,
	}
	...
	log.Println("handleUDPConnection 收到数据", tmpPort, dst.String(), dstIP)
...
func ReadAndHandleUDPReq(c *SecurePacketConn) error {
	buf := leakyBuf.Get()              // 新建内存空间
	n, src, err := c.ReadFrom(buf[0:]) // 接收数据
	if err != nil {
		return err
	}
	log.Println("ReadAndHandleUDPReq 收到数据 长度=", n)
